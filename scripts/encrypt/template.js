...
const PLACEHOLDERS = [
  ...
  // Multi-level placeholders (additive — single-password templates ignore them)
  { token: '{{hbeLevels}}',            field: 'levelsJSON',     mode: 'attr' },
  { token: '{{hbeMinLevel}}',          field: 'minLevel',       mode: 'attr' },
  { token: '{{hbeLevelCiphers}}',      field: 'levelCiphers',   mode: 'raw' },
  { token: '{{hbeIntegrity}}',         field: 'integrityJSON',  mode: 'attr' },
  { token: '{{hbePageCorruptMessage}}',field: 'corruptMsg',     mode: 'text' },
  { token: '{{hbeLevelPassMessage}}',  field: 'levelPassMsg',   mode: 'text' },
];
...
function substitute(template, opts) {
  ...
  for (const { token, field, mode } of PLACEHOLDERS) {
    ...
    if (mode === 'attr') replacement = escapeAttr(raw);
      ...
      // Raw mode: insert verbatim. Used for pre-rendered HTML blocks
      // (e.g. per-level ciphertext script tags) that are already
      // properly escaped by the caller.
      replacement = raw;
    }
    else if (mode === 'hex') {
